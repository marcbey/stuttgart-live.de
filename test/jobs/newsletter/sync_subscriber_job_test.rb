...
class Newsletter::SyncSubscriberJobTest < ActiveJob::TestCase
  test "delegates to mailjet sync service" do
    subscriber = NewsletterSubscriber.create!(email: "job@example.com", source: "homepage", confirmed_at: Time.current)
  ...
  test "skips unconfirmed subscribers" do
    subscriber = NewsletterSubscriber.create!(email: "pending-job@example.com", source: "homepage")
    captured_subscriber = nil
    ...
    assert_nil captured_subscriber
  ensure
    sync_class.alias_method :call, :__original_call_for_test
    sync_class.remove_method :__original_call_for_test
  end
