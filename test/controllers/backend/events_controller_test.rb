...
class Backend::EventsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "show keeps active inbox status in editor form" do
    ...
    assert_select "input[name='event[show_venue_description]']", count: 0
    ...
    assert_select "#event-editor-panel-event trix-editor.backend-description-editor[data-action*='trix-attachment-add->description-image-upload#upload']", count: 1
    assert_select "#event-editor-panel-event .form-hint", text: /Bilder im Text\./, count: 1
    ...
    assert_select "#event-editor-panel-event .form-hint", text: /Automatisch veröffentlicht werden nur Events mit Status „Unpublished“, die veröffentlichungsreif sind\./, count: 1
  ...
  test "show populates ticket url with the frontend ticket url" do
    ...
    assert_select "label.form-label", text: "Ticket Sonderlocke"
    assert_select "input#ticket_special_note_event_#{@published_event.id}[name='event[ticket_special_note]']"
  ...
  test "turbo frame new shows the same event fields with read only promoter display" do
    ...
    assert_select "input[name='event[ticket_special_note]']"
  ...
  test "updates event" do
    ...
    assert_equal "Bitte beim Veranstalter nach Restkarten fragen", @event.ticket_special_note
