class SeedLlmEnrichmentPromptTemplateSetting < ActiveRecord::Migration[8.1]
  ...
  def up
    AppSettingMigration.find_or_create_by!(key: "llm_enrichment_prompt_template") do |setting|
      setting.value = LLM_ENRICHMENT_PROMPT_TEMPLATE
    end
  end

  def down
    AppSettingMigration.where(key: "llm_enrichment_prompt_template", value: LLM_ENRICHMENT_PROMPT_TEMPLATE).delete_all
  end
end
