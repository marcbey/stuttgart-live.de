class UpdateExistingOrganizerNotesToNewSecurityCopy < ActiveRecord::Migration[8.1]
  ...
  def up
    EventMigration.where(organizer_notes: [ OLD_NOTES, INTERMEDIATE_NOTES ]).update_all(organizer_notes: NEW_NOTES)
  end

  def down
    raise ActiveRecord::IrreversibleMigration, "existing organizer notes were updated to the new security copy"
  end
end
