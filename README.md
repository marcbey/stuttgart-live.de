...
- Pro Quelldatensatz wird ein `RawEventImport` angelegt. Innerhalb der aktiven Retention bleibt diese Rohhistorie append-only; der tägliche Retention-Job behält aber immer den neuesten Rohimport pro Quelle und `source_identifier` und löscht ältere Versionen nach 14 Tagen, wenn bereits ein neuerer Snapshot vorhanden ist.
...
Für den täglichen Betrieb startet `Importing::DailyRunJob` die aktiven Provider standardmäßig über `config/recurring.yml` jeden Tag um `03:05` Uhr. Der tägliche Merge läuft anschließend über `Merging::DailyRunJob` standardmäßig um `04:05` Uhr. Das tägliche LLM-Enrichment wird separat über `Importing::LlmEnrichment::DailyRunJob` standardmäßig um `05:05` Uhr eingereiht. Diese drei täglichen Wrapper-Jobs bleiben in `config/recurring.yml` statisch definiert, lassen sich im Backend unter `Queue` aber jeweils per Checkbox aktivieren oder deaktivieren; manuelle Starts bleiben davon unberührt. Danach räumt `Events::Retention::PruneStaleUnpublishedEventsJob` um `06:05` Uhr nicht veröffentlichte Events auf, deren `start_at` länger als einen Monat zurückliegt; `Events::Retention::PrunePastRawEventImportsJob` löscht anschließend um `06:35` Uhr alte `raw_event_imports`. Dabei werden erst überholte Rohimport-Snapshots gelöscht, wenn sie älter als 14 Tage sind und ein neuerer Snapshot mit derselben Quelle und `source_identifier` existiert; pro Lauf ist diese Bereinigung begrenzt, damit große Aufräumläufe die Datenbank nicht unnötig belasten. Anschließend löscht derselbe Job weiterhin Rohimporte für Events, deren normalisiertes Eventdatum länger als einen Monat vorbei ist. Historische Event-Detailseiten bleiben dabei erhalten, weil die Public-Seite aus `events` liest und der Merge die relevanten Quellinformationen in `events.source_snapshot` ablegt. Manuell lassen sich die Provider-Läufe im Backend unter den Importquellen oder per Rake-Task starten:
...
Zusätzlich zum exakten Dublettenschlüssel gibt es ein optionales Ähnlichkeits-Matching für Artist-Namen und prägnante Titelvarianten bei gleicher oder sehr naher Startzeit. Damit kann der Merge-Import Fälle wie `Vier Pianisten - Ein Konzert` und `Vier Pianisten`, `Gregory Porter & Orchestra` und `Gregory Porter` oder einen kurzen Veranstaltungstitel innerhalb eines längeren Provider-Titels als denselben Termin erkennen. Titel-Varianten werden dabei nur bei passender Venue zusammengeführt. Dieses Verhalten lässt sich im Backend unter `Einstellungen` über das Setting `Ähnlichkeits-Matching für Event-Dubletten` ein- oder ausschalten.
...
2. Jedes Event wird einzeln über die OpenAI Responses API verarbeitet; pro Event gibt es genau einen OpenAI-Call.
...
5. Das LLM liefert `genre`, `event_description`, `venue_description`, `venue_address`, `venue_external_url` sowie die vier Search-Linkfelder als Structured Output zurück. Die Struktur wird technisch per JSON Schema erzwungen; der Prompt bleibt die fachliche Anleitung. Nur die vier Search-Linkfelder dürfen aus den mitgelieferten Suchtreffern gewählt werden; `venue_external_url` kommt direkt aus Prompt und LLM-Response.
...
8. Der Lauf protokolliert Auswahlmenge, übersprungene Events, erfolgreiche Enrichments, abgeschlossene OpenAI-Calls, Web-Search-Metriken, Cursor-Metadaten und Fehler im zugehörigen `ImportRun`. Zusätzlich speichert der Job eine API-Call-Liste mit OpenAI-Requests, Web-Search-Requests und Web-Search-Cache-Hits, die auf der Job-Detailseite sichtbar ist.
...
- Bei LLM-Enrichment-Runs zeigt die Job-Detailseite zusätzlich die dokumentierten API Calls. Dort sind OpenAI-Aufrufe, Web-Search-Aufrufe und Web-Search-Cache-Hits mit Provider, Operation, Status, Event-Bezug, Dauer und Payload-Details sichtbar.
...
Die Tracking-Einwilligung benennt die Südwestdeutsche Konzertdirektion Erwin Russ GmbH als Verantwortliche und Mailjet als eingesetzten Newsletter-Dienst. Sie weist ausdrücklich darauf hin, dass die Einwilligung freiwillig ist und separat über die Tracking-Einstellungen oder durch Mitteilung an StuttgartLIVE widerrufen werden kann, ohne den Newsletter abzubestellen.
...
Newsletter-Formulare verwenden die Referrer-Policy `same-origin`: Externe Seiten erhalten keinen Referrer, interne Formularanfragen behalten die für den Rails-CSRF-Schutz nötige Herkunftsangabe. `no-referrer` darf hier nicht verwendet werden, weil Browser sonst `Origin: null` senden und Rails die Anmeldung mit 422 ablehnt. Die Formular-Regressionstests prüfen diese Abläufe ausdrücklich mit aktiviertem CSRF-Schutz. Allgemeine Fehlerseiten verwenden ein schlankes eigenes Layout ohne die alte Navigation, Analyse-Skripte oder datenbankabhängige Inhaltsbereiche.
...
- `merge_artist_similarity_matching_enabled` für das quellenübergreifende Ähnlichkeits-Matching von Artist-Namen und prägnanten Titelvarianten im Merge-Import bei gleicher oder sehr naher Startzeit
...
- optional `RUSS_LIVE_PRIMARY_DB_PASSWORD` und `RUSS_LIVE_OPERATIONAL_DB_PASSWORD`, wenn du das Host-Setup für `russ-live.de` per Ansible ergänzen willst
...
Für `russ-live.de` legt das Host-Setup zusätzlich getrennte Operational-Datenbanken an:
