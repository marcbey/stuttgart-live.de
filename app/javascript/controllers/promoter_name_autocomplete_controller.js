...
export default class extends Controller {
  static targets = ["input", "panel", "results", "status"]
  static values = {
    url: String,
  ...
  async fetchResults() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Promoter name search failed with status ${response.status}`)
      }

      this.results = (await response.json()).map((result) => ({ name: result.name || result }))
      this.activeIndex = this.results.length > 0 ? 0 : -1
      this.renderResults()
    } catch (error) {
      if (error.name === "AbortError") return

      this.renderHint("Promoter-Suche ist gerade nicht verfügbar")
    } finally {
  ...
  renderResults() {
    ...
    if (this.results.length === 0) {
      this.renderHint("Keine Treffer. Der eingegebene Name wird beim Speichern übernommen.")
      return
    }
    ...
    const itemsHtml = this.results.map((result, index) => {
      ...
      return `
        <button type="button"
                class="promoter-name-autocomplete-item${activeClass}"
                data-index="${index}"
                data-action="click->promoter-name-autocomplete#select">
          <span class="promoter-name-autocomplete-item-name">${this.escapeHtml(result.name)}</span>
        </button>
      `
    }).join("")

    this.resultsTarget.innerHTML = itemsHtml
    this.statusTarget.textContent = `${this.results.length} Promoter-Vorschläge verfügbar`
  }

  renderHint(message) {
    this.panelTarget.hidden = false
    this.inputTarget.setAttribute("aria-expanded", "true")
    this.resultsTarget.innerHTML = `<div class="promoter-name-autocomplete-empty">${this.escapeHtml(message)}</div>`
