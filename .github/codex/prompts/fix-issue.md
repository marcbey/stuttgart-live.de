# Codex Issue Fix

You are working inside this repository from a GitHub Actions job.

Implement the smallest correct fix for the issue described below.

Requirements:
- Keep the diff narrow and avoid unrelated refactoring.
- Add or update tests if that is appropriate to prove the fix.
- Leave the branch in a PR-ready state.
- Do not change dependencies, tooling, or documentation unless the issue clearly requires it.
- If the issue is unclear, avoid unsafe guessing. Make only a clearly justified minimal change, or stop and explain the blocker in your final message.

Issue title:
{{ISSUE_TITLE}}

Issue body:
```text
{{ISSUE_BODY}}
```
